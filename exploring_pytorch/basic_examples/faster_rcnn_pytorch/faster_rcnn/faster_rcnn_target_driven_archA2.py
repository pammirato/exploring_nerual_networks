...
class FasterRCNN(nn.Module):
    ...
    def __init__(self, classes=None, debug=False):
        ...
        self.fc6 = FC(512 * roi_size * roi_size*2, 4096)
    ...
    def forward(self, target_data, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        ...
        target_roi = network.np_to_variable(
                            np.array([0,0,0,target_data.shape[1],target_data.shape[2]]),
                            is_cuda=True).unsqueeze(0)
        target_pooled = self.roi_pool(target_features,target_roi)
        target_x = target_pooled.view(target_pooled.size()[0],-1)

        x = torch.cat([x,target_x.expand_as(x)], dim=1)
