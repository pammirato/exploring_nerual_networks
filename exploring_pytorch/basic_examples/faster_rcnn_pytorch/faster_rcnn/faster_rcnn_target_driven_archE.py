...
class RPN(nn.Module):
    ...
    def __init__(self):
        ...
        self.batch_norm_diffs = torch.nn.BatchNorm2d(1)

        #self.conv1 = Conv2d(512, 512, 3, same_padding=True)
        self.conv1 = Conv2d(1, 1, 5, same_padding=True)
        #self.score_conv = Conv2d(512, len(self.anchor_scales) * 3 * 2, 1, relu=False, same_padding=False)
        self.score_conv = Conv2d(1, len(self.anchor_scales) * 3 * 2, 1, relu=False, same_padding=False)
        #self.bbox_conv = Conv2d(512, len(self.anchor_scales) * 3 * 4, 1, relu=False, same_padding=False)
        self.bbox_conv = Conv2d(1, len(self.anchor_scales) * 3 * 4, 1, relu=False, same_padding=False)
    ...
    @property
    def loss(self):
        return self.cross_entropy + self.loss_box * 10 + self.feature_extraction_loss*20
    ...
    def forward(self, target_data, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        ...
        ones_kernel = network.np_to_variable(np.ones((1,512,ts,ts)))
        ...
        normed_diffs = self.batch_norm_diffs(feat_diffs)


        rpn_conv1 = self.conv1(normed_diffs)
        ...
        if self.training:
            self.feature_extraction_loss = self.feature_extraction_loss_layer(
                                                normed_diffs, features, target_features, 
                                                gt_boxes,
                                                self._feat_stride)
    ...
    def feature_extraction_loss_layer(normed_diffs, image_features, 
        ...
        if gt_box is not None and gt_box.shape[0]>0 and gt_box[0,4] > 0:
            ...
            target_feat_loss = (normed_diffs[0,0,gt_center[1],gt_center[0]]**2) * 5
            ...
            #    gt_cols = range(img_feat_box[0],img_feat_box[2])
        ...
        min_row_indexes = feat_diffs.argmin(0) 
        min_col_indexes = np.arange(min_row_indexes.shape[0])
        #don't consider places with the gt box
        if gt_center:
            min_diffs = np.delete(min_diffs, gt_center[1])
            min_row_indexes = np.delete(min_row_indexes, gt_center[1]) 
            min_col_indexes = np.delete(min_col_indexes, gt_center[1]) 
        ...
        bg_feat_loss = torch.max(network.np_to_variable(np.array([0])), .25 - (normed_diffs[0,0,chosen_min_row,chosen_min_col]**2))


        return  .5*(target_feat_loss + bg_feat_loss)
