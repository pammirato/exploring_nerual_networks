...
output_dir = ('/playpen/ammirato/Data/Detections/' + 
             '/saved_models/')
save_name_base = 'PC_1-5_archA_2'
save_freq = 1 

trained_model_path = ('/playpen/ammirato/Data/Detections/' +
                     '/saved_models/')
trained_model_name = 'PC_1-5_archA_0_14_0.09861.h5'
...
# load net
net = Part_Classifier(classes=train_set.get_class_names())
...
for epoch in range(num_epochs):
    ...
    for step,batch in enumerate(trainloader):
        ...
        #box_ind = int(np.random.choice(gt_boxes.shape[0]))
        #if gt_boxes.shape[0] > 1 and box_ind == 0:
        #    if np.random.rand() > .2:
        #        box_ind = np.random.choice(gt_boxes.shape[0] -1) + 1 
        #gt_boxes = gt_boxes[box_ind,:-1]


        #if gt_boxes[3] - gt_boxes[1] < 20:
        #    print 'skip'
        #    continue
        #if gt_boxes[2] - gt_boxes[0] < 20:
        #    print 'skip'
        #    continue
        #
        #targets_cnt[gt_boxes[4]][0] += 1

        #im_data = im_data[:,gt_boxes[1]:gt_boxes[3],gt_boxes[0]:gt_boxes[2],:]
