...
def test_net(name, net, dataloader, max_per_image=300, thresh=0.05, vis=False,
    ...
    for i,batch in enumerate(dataloader):
        ...
        scores, boxes = im_detect(net, im_data,im_info)
        ...
        #only keep one box per instance
        for j in xrange(1, dataloader.dataset.get_num_classes()):
            class_boxes = all_boxes[j][i]
            if class_boxes.shape[0] >1:#if there is more than 1 box
                #pick the highest scoring box
                max_score = np.max(class_boxes[:,-1])
                keep = np.where(class_boxes[:,-1] >= (max_score -.00001))[0]
                all_boxes[j][i] = all_boxes[j][i][keep,:]
