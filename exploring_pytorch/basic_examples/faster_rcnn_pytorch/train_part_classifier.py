...
for epoch in range(num_epochs):
    ...
    for step,batch in enumerate(trainloader):
        ...
        loss = net.loss 
        #loss *= class_weights[target_box[4]]
    ...
    if epoch % save_freq == 0:
        if load_trained_model:
            save_name = os.path.join(output_dir, save_name_base+'_{}_{:1.5f}.h5'.format(
                                                  epoch+trained_epoch+1, epoch_loss/(step+1)))
        else:
