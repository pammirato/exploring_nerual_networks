...
def test_net(name, net, dataloader, max_per_image=300, thresh=0.05, vis=False,
    ...
    feat_name = base_name + model_name + '_features.txt'
    label_name = base_name + model_name + '_labels.txt'

    #feat_fid = open('/playpen/ammirato/Data/Detections/features/' +  
    #                 name + '_features.txt', 'w')
    #
    #label_fid = open('/playpen/ammirato/Data/Detections/features/' +  
    #                 name + '_labels.txt', 'w')
    ...
    for i,batch in enumerate(dataloader):
        ...
        # forward
        #features, class_scores = net(im_data,gt_boxes)
        features = net(im_data,gt_boxes)

        #print features.data.cpu().numpy().min()
        #print features.data.cpu().numpy().max()
        #print np.abs(features.data.cpu().numpy()).mean()
        #print 'hasdfasdfasdfsd'
        ...
        if obj_x_locs.shape[0] < 1:
            print 'Skip'
            continue

        obj_x = int(np.random.choice(obj_x_locs))
        obj_y = int(np.random.choice(obj_y_locs))
        bg_x = int(np.random.choice(bg_x_locs))
        bg_y = int(np.random.choice(bg_y_locs))

        obj_feats = features[0,:,obj_y,obj_x]
        bg_feats = features[0,:,bg_y,bg_x]

        all_features[i*2,:] = obj_feats
        all_features[i*2 + 1,:] = bg_feats

        all_labels[i*2,:] = gt_boxes[1,4]
        all_labels[i*2 +1,:] = gt_boxes[0,4]

        #feat_fid.write(np.array_str(obj_feats, max_line_width=100000)[3:-1])
        #feat_fid.write('\n')
        #feat_fid.write(np.array_str(bg_feats, max_line_width=100000)[3:-1])
        #feat_fid.write('\n')

        #label_fid.write(str(gt_boxes[1,4]))
        #label_fid.write('\n')
        #label_fid.write(str(gt_boxes[0,4]))
        #label_fid.write('\n')
